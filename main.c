...
void put_block_at_field(tetrimino_type t, field_type *f, int block)
{
...
void save_at_field(tetrimino_type t, field_type *f)
{
#ifdef DEBUG
    FILE *file;
#endif
...
int is_tetr_fit(const tetrimino_type t, field_type f)
{
    int i, ty, tx;
    for(i = 0; i < blocks_count; i++) {
...
int is_tetr_on_something(const tetrimino_type t, field_type f)
{
    int i, ty, tx;
    for(i = 0; i < blocks_count; i++) {
        ty = get_tetr_block_crd(t, i, crd_y);
...
int tetr_fall(tetrimino_type *t, field_type f)
{
	t->pos.y++;
...
void tetr_rotate(tetrimino_type *t, field_type f)
{
	t->rotation++;
...
void tetr_move(tetrimino_type *t, field_type f, int dir)
{
    t->pos.x += dir;
    if(!is_tetr_fit(*t, f)) {
        t->pos.x -= dir;
...
void field_rewrite(WINDOW *win, field_type f)
{
    int i, j;
    for(i = 0; i < fh; i++) {
        for(j = 0; j < fw; j++) {
...
void field_remove_line(field_type *f, int n)
{
    int i;
    for(i = 0; i < fw; i++)
        (*f)[n][i] = 0;
...
void field_shift(field_type *f, int n)
{
    int i, j;
...
int check_field_lines(field_type *f)
{
    int i, n = 0;
...
void init_game_field(field_type *f)
{
    int i, j;
    for(i = 0; i < fh; i++)
        for(j = 0; j < fw; j++) 
	...
	while((key = getch()) != key_escape) {
        ...
        if(dcount == delay_times[level] || dflag) {
            if(tetr_fall(&curr_tetr, field)) {
                ...
                curr_tetr = next_tetr;
                tetr_init(&next_tetr, rand()%shapes_count);
                flines = check_field_lines(&field);
                score += flines == 4 ? flines*2000*score_mod : flines*500*score_mod;
                if(flines > 0) {
                    field_rewrite(win, field);
                };
                dflag = 0;
            }
